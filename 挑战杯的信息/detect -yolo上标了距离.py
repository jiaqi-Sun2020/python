...
def get_min_distance(xyxy,img):
    ...
    img = cv2.cvtColor(img,cv2.COLOR_RGB2GRAY)  #灰度图
    h = int(xyxy[3])-int(xyxy[1])  #y2-y1
    w = int(xyxy[2])-int(xyxy[0])  #x2-x1

    #print(xyxy)
    roi = img[int(xyxy[1]+h/4):int(xyxy[3]-h/4),int(xyxy[0]+w/4):int(xyxy[2]-w/4)]  #缩小roi防止获取到另外的区域
    max_ = np.max(roi)   #获取roi区域最大的灰度值,放入距离公式进行计算
    # print(roi.shape)
    # print(max_)
    # if(max_>200):
    #     y=999
            ...
            if det is not None and len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or view_img:  # Add bbox to image
                        ...
                        xyxy[1] = xyxy[1]-120
                        xyxy[3] = xyxy[3]-120
                        if 1200<xyxy[0]<1210 and 70<xyxy[1]<80 and 1404<xyxy[2]<1414 and 201<xyxy[3]<211:     #[1205, 75, 1409, 206]  为了过滤掉视频上的证件
                             continue

                        if names[int(cls)]=="kites":   #过滤掉不同的标签
                            continue

                        if xyxy[3]<0 or xyxy[1]<0 :  #当roi为复数的时候直接跳过
                            # xyxy[1] = xyxy[1] +120
                            # xyxy[3] = xyxy[3] + 120
                            continue


                        distance =get_min_distance(xyxy,img)  #获取距离
                        #print(distance)
                        xyxy[1] = xyxy[1] + 120
                        xyxy[3] = xyxy[3] + 120
